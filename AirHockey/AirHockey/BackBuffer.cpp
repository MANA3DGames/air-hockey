...
BackBuffer::BackBuffer( HWND hWnd, int width, int height )
{
	...
	// Create the backbuffer surface bitmap that is compatible with the window device context bitmap format.
	// That is the surface we will render onto.
	mhSurface = CreateCompatibleBitmap( hWndDC, width, height );

	// Done with window DC.
	ReleaseDC( hWnd, hWndDC );

	// At this point, the back buffer surface is uninitialized, so lets clear it to some non-zero value. Note that it
	// needs to be non-zero. If it is zero then it will mess up our sprite blending logic. 
